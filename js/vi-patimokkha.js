...
function lookupWordHandler(event) {
    ...
    // --- Direct Dictionary Lookup (for both Normal and Recitation modes) ---
    showDictionary();
}
...
// --- ADD THIS NEW CODE ---
    ...
    if (isLoopActive && displayArea.classList.contains('recitation-active')) {
        ...
        // Cập nhật thông báo SRS khi app vừa tải xong
        updateReviewBadge();
    }
...
function openSRSList(type) {
    ...
    // Đặt tiêu đề
    const titleMap = {
        'due': 'Cần ôn tập hôm nay',
        'learning': 'Đang học',
        'mastered': 'Đã thuộc kỹ',
        'new': 'Chưa học'
    };
    ...
    if (itemsFound === 0) {
        listContent.innerHTML = '<div style="text-align: center; color: #9ca3af; padding: 20px;">Không có câu nào trong danh sách này.</div>';
    }
}
...
function injectSlider(lineElement, idx) {
    ...
    if (!container) {
        ...
        const againLabel = startMins >= 60 ? `Học Lại` : `Học Lại`;

        // Define the 4 rating buttons dynamically
        const ratings = [
            { val: 2, label: 'Khó', color: '#c0392b' },
            { val: 3, label: 'TB', color: '#27ae60' },
            { val: 4, label: 'Dễ', color: '#2980b9' }
        ];
...
function finishQuizSuccess() {
    ...
    let xpEarned = quizQueue.length; // Default base
        alert(`Hoàn thành! Bạn nhận được +${quizQueue.length} XP.`);
